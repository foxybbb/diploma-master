...
class Draw:
    def __init__(self, max_range_m, num_ant, num_samples):
        ...
        self._vertical_lines = []  # Store vertical lines
        self._peak_vertical_lines = []  # Store peak vertical lines
    ...
    def _draw_first_time(self, data_all_ant, peaks, peak_value=None):
        for ant_idx in range(self._num_ant):
            ...
            vline = self._axs[ant_idx].axvline(x=3.0, color='r', linestyle='--', alpha=0.5, label='Range Limit')
            ...
            if peaks[ant_idx] is not None:
                ...
                peak_vline = self._axs[ant_idx].axvline(x=peak_dist, color='g', linestyle='-', alpha=0.5, label='Peak Position')
            ...
            if peak_value is not None:
                self._peak_line = self._axs[ant_idx].axhline(y=peak_value, color='g', linestyle='-', label=f'Peak Threshold: {peak_value:.2e}')
            ...
            self._axs[ant_idx].set_xlim(0, self._xlim)  # Keep full x-axis range
    ...
    def _draw_next_time(self, data_all_ant, peaks, peak_value=None):
        for ant_idx in range(self._num_ant):
            ...
            # Update main signal with masked data
            self._pln[ant_idx].set_ydata(masked_data)
            
            # Update peak vertical line
            ...
            # Update peak threshold if available
            if peak_value is not None:
